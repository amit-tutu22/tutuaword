//! U-F12-S2 — diagram preview PNG also populates shape selection batch.
...
#[test]
fn u_f12_s2_diagram_preview_shape_selection_batch() {
    ...
    assert!(list.image_batch.payloads.iter().any(|p| p.as_slice() == PNG_1X1));
}
