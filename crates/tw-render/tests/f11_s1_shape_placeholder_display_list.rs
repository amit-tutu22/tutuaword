//! F11.S1 — imported shapes emit a placeholder rect in the display list.
...
#[test]
fn u_f11_s1_shape_placeholder_rect_in_display_list() {
    ...
    for (idx, chunk) in list.rect_batch.rects.chunks(4).enumerate() {
        ...
        if color == SHAPE_PLACEHOLDER_COLOR
            && (chunk[2] - 120.0).abs() < 0.01
            && (chunk[3] - 80.0).abs() < 0.01
        {
            found_fill = true;
            break;
        }
    }
    assert!(found_fill, "expected shape placeholder fill rect");
}
