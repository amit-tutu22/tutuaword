...
- Export is reachable from the worker via `BridgeCommand::ExportPdf`; Flutter has `EditorController.exportPdf()` but **no ribbon or menu entry** yet.
...
| `app/lib/editor/editor_controller.dart` | `exportPdf()` (no UI caller) |
