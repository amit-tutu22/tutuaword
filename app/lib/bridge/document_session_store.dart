...
class DocumentSessionStore {
  ...
  /// U-F01-S2-autosave-serializes: write draft bytes + manifest metadata.
  ...
  Future<AutosaveSnapshot?> readAutosave() async {
    ...
    try {
      final manifest = jsonDecode(await _manifestFile.readAsString()) as Map<String, dynamic>;
  ...
  /// Legacy helper — returns paths only (no bookmarks).
  ...
  List<String> bumpRecentPath(List<String> current, String path) {
    ...
    final next = [normalized, ...current.where((item) => p.normalize(item) != normalized)];
    return next.take(maxRecentFiles).toList();
  }
  ...
  Duration loadAutosaveInterval() {
    ...
    try {
      final decoded = jsonDecode(_settingsFile.readAsStringSync()) as Map<String, dynamic>;
